...
- "feedback"：用户想要查询用户反馈。关键词包括：反馈、用户反馈、问题反馈、投诉、用户问题、bug反馈、feedback等。需要提取 feedback_keyword（反馈搜索关键词）和可选的 feedback_days（查询天数，默认3天）。
- "unknown"：无法归类为以上四种的闲聊或无关内容。
...
{
  "intent": "record" | "query" | "delete" | "feedback" | "unknown",
  "fields": {
    "app_name": "",
    "scene": "",
    "big_island": "",
...
当 intent 为 "feedback" 时，只需填写 feedback_keyword 和 feedback_days，其他字段为空。
...
class LLMService:
    ...
    async def summarize_feedback(self, keyword: str, total: int, feedback_texts: list[str]) -> dict:
        """用 LLM 对用户反馈做分类汇总，返回结构化数据"""
        sample_count = len(feedback_texts)
        prompt = f"""以下是从小米反馈平台搜索「{keyword}」得到的共 {total} 条用户反馈中的 {sample_count} 条样本。
请对这些反馈进行分类汇总。

要求：
1. 将反馈归类为不同的问题分类（如"点击无反应"、"找不到设置入口"、"功能建议"等）
2. 统计每个分类在样本中的数量，然后按比例推算到总数 {total}
3. 占比 = 推算数量 / 总数 {total}，四舍五入到整数百分比
4. 用一句话简短概括主要原因（不超过20字）
5. 按数量从多到少排列，尽可能细分为不同类别，至少输出 15 个分类
6. 不要合并为"其他"，每个问题类型都单独列出

输出严格 JSON 格式：
{{"categories": [{{"name": "分类名", "count": 数量, "percent": "占比%", "reason": "主要原因"}}]}}

反馈内容：
"""
        # 限制每条反馈长度，避免 token 超限
        for i, t in enumerate(feedback_texts, 1):
            if len(t) > 80:
                t = t[:80]
            prompt += f"{i}. {t}\n"

        print(f"[LLM] 反馈汇总 prompt 长度: {len(prompt)} 字符, 样本数: {sample_count}")
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                resp = await client.post(
                    ...
                    json={
                        ...
                        "messages": [
                            {"role": "system", "content": "你是一个用户反馈分析助手。输出严格的 JSON 格式。"},
